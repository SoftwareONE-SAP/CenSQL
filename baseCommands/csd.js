...
CsMemDeltaCommandHandler.prototype.run = function(command, cParts, conn, screen, callback){
	...
	conn.exec("conn", "SELECT YEAR(SNAPSHOT_ID), MONTH(SNAPSHOT_ID), DAYOFMONTH(SNAPSHOT_ID), HOUR(SNAPSHOT_ID), 'Instance', SUM(MEMORY_SIZE_IN_DELTA), MIN(SNAPSHOT_ID)\
        ...
        callback([err == null ? 0 : 1, err == null ? data : err, err == null ? "line-graph" : "json", "CS in memory delta over the last 3 days", 3 * 24]);
    })
