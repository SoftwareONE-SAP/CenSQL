...
CapacityGraphCommandHandler.prototype.run = function(command, cParts, conn, screen, callback){
	...
	conn.exec("conn", 
		"SELECT \
			CAST(MAX(R.USED_SIZE) AS BIGINT) AS ROW_USED_SIZE, \
			SUM(CAST(C.MEMORY_SIZE_IN_MAIN AS BIGINT) + CAST(C.MEMORY_SIZE_IN_DELTA AS BIGINT) + CAST(C.MEMORY_SIZE_IN_HISTORY_MAIN AS BIGINT) + CAST(C.MEMORY_SIZE_IN_HISTORY_DELTA AS BIGINT)) AS TOTAL_CS_IN_MEM, \
			SUM(\
				CASE \
					WHEN C.LOADED = 'FULL' \
						THEN 0 \
