...
CsMemDeltaCommandHandler.prototype.run = function(command, cParts, conn, screen, callback){
	...
	conn.exec("conn", "SELECT YEAR(SNAPSHOT_ID), MONTH(SNAPSHOT_ID), DAYOFMONTH(SNAPSHOT_ID), HOUR(SNAPSHOT_ID), 'Instance', TO_INTEGER(SUM(MEMORY_SIZE_IN_DELTA)/1024/1024/1024), MIN(SNAPSHOT_ID)\
        ...
        callback([err == null ? 0 : 1, err == null ? data : err, err == null ? "line-graph" : "json", "CS in memory delta GB over the last 3 days", 3 * 24]);
    })

}

module.exports = CsMemDeltaCommandHandler;